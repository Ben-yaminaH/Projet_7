...
// Vider le contenu de recipesSection pour éviter les duplications
...
function filterData() {
    ...
    filterData3(ingredient_array);
      ...
      // Met à jour les options dans les sélecteurs en fonction des résultats filtrés
...
function createSelect() {
    ...
    for (let i = 0; i < dataArray.length; i++) {
        ...
        for (let x = 0; x < dataArray[i].ingredients.length; x++) {

            let ingredient_exist = "false";
            for (let y = 0; y < array_ingredients.length; y++) {
...
function updateSelectOptions(data) {
    updateIngredientsOptions(data);
    updateApplianceOptions(data);
    updateUstensilsOptions(data);
}

function updateIngredientsOptions(data) {
...
// Mise à jour des options d'appareils
function updateApplianceOptions(data) {
    const sel = document.getElementById("select2");
    sel.innerHTML = ""; 
...
// Mise à jour des options d'ustensiles
function updateUstensilsOptions(data) {
...
function updateRecipeCount(count) {
    const totalRecipesDiv = document.querySelector(".total_recipes .nbr_recette");
    totalRecipesDiv.textContent = `${count} recette${count > 1 ? 's' : ''}`;
}